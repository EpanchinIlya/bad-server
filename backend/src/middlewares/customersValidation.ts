...
export const customerValidation = {
  ...
  getCustomers: celebrate({
    [Segments.QUERY]: Joi.object({
      ...
      sortField: Joi.string().valid('createdAt', 'totalAmount', 'orderCount', 'lastOrderDate', 'name', 'email').optional().default('createdAt'),
  ...
  updateCustomer: celebrate({
    ...
    [Segments.BODY]: Joi.object({
      ...
      .messages({
        'object.min': 'Тело запроса не может быть пустым для обновления клиента.',
        'object.unknown': 'Недопустимое поле в теле запроса.',
        'string.min': 'Поле "{#label}" должно содержать как минимум {#limit} символа.',
        'string.max': 'Поле "{#label}" не должно превышать {#limit} символов.',
